...
public class AuctinoCursorTest {
    ...
    void findAllQueryDSL() {
        // 현재가 기준 내림 차순
        List<AuctionDto> auctions = auctionService.auctionListQueryDSL(null, null, null);
    ...
    void findAuctionByStatus() {
        List<AuctionDto> auctionsProgress = auctionService.auctionListQueryDSL(null, null, "progress");
        Assertions.assertThat(auctionsProgress.size()).isEqualTo(2);

        List<AuctionDto> auctionsClosed = auctionService.auctionListQueryDSL(null, null, "closed");
    ...
    void findAuctionOrderBy() {
        ...
        List<AuctionDto> orderByCurrentPrice = auctionService.auctionListQueryDSL(null, "currentPrice", null);
        Assertions.assertThat(orderByCurrentPrice.get(0).getId()).isEqualTo(1035L);

        //given - order by StartDate
        List<AuctionDto> orderByStartDate = auctionService.auctionListQueryDSL(null, "startDate", null);
        Assertions.assertThat(orderByStartDate.get(1).getId()).isEqualTo(1036L);

        //given - order by endDate
        List<AuctionDto> orderByEndDate = auctionService.auctionListQueryDSL(null, "endDate", null);
